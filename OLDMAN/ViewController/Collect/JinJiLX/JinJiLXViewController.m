//
//  JinJiLXViewController.m
//  OLDMAN
//
//  Created by 赵晓东 on 16/1/15.
//  Copyright © 2016年 ZXD. All rights reserved.
//

#import "JinJiLXViewController.h"
...
@interface JinJiLXViewController ()<UITextFieldDelegate,UIScrollViewDelegate,ZHPickViewDelegate>
{
...
@implementation JinJiLXViewController
...
-(void)loadData
{
    ...
    [_collectViewModel SelectAssessmentResult:self.shenFenZJ tableFlag:@"38"];
    ...
    [_collectViewModel setBlockWithReturnBlock:^(id returnValue) {
        ...
        if ([returnValue[@"success"] intValue]==1) {
            ...
            if ([returnValue[@"data"] isKindOfClass:[NSDictionary class]]) {
                ...
                UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"监护人信息同步到紧急联系人" message:nil delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
...
-(void)createNav
{
    ...
    self.navigationItem.title = @"紧急联系人信息";
...
-(void)JieShao_Button_Click:(UIButton*)button
{
    switch (button.tag) {
        ...
        case 90003:
        {
            RNBlurModalView * modal = [[RNBlurModalView alloc] initWithViewController:self title:nil message:@"记录：须向被访者询问其联系人的真实住宅电话，如实填写，无需填写区号，直接填写电话号码为7或8位数字。填写后须向被访者进行确认，确保填写准确无误。住宅和移动电话至少填写一项。"];
        ...
        case 90004:
        {
            RNBlurModalView * modal = [[RNBlurModalView alloc] initWithViewController:self title:nil message:@"记录：须向被访者询问其联系人的真实移动电话，如实填写，直接填写11位数字号码。填写后须向被访者进行确认，确保填写准确无误。住宅和移动电话至少填写一项。"];
        ...
        case 90005:
        {
            RNBlurModalView * modal = [[RNBlurModalView alloc] initWithViewController:self title:nil message:@"记录：须按被访者联系人现住址如实填写。填写后须向被访者进行确认，确保填写准确无误。"];
        ...
        case 90006:
        {
            RNBlurModalView * modal = [[RNBlurModalView alloc] initWithViewController:self title:nil message:@"记录：须向被访者联系人询问其最常用电子邮箱，如实填写。填写后须向被访者联系人进行确认，确保填写准确无误。如没有可以不用填写。"];
...
#pragma mark - 监护人名
        UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"请输入“紧急联系人姓名”" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alertView show];
...
-(void)update_JinJiLX
{
    ...
    [parameter setObject:@"38" forKey:@"tableFlag"];
