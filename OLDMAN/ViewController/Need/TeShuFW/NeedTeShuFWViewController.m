//
//  NeedTeShuFWViewController.m
//  OLDMAN
//
//  Created by 赵晓东 on Title_text_font/1/11.
//  Copyright © 20Title_text_font年 ZXD. All rights reserved.
//

#import "NeedTeShuFWViewController.h"
...
@interface NeedTeShuFWViewController ()<UITextFieldDelegate,UIScrollViewDelegate>
{
...
@implementation NeedTeShuFWViewController
...
-(void)loadData
{
    ...
    [_collectViewModel SelectAssessmentResult:self.shenFenZJ tableFlag:@"26"];
...
-(void)createView
{
    ...
    //选项标题
    UILabel * xuanXiang_Label=[ZCControl createLabelWithFrame:CGRectMake(CGRectGetMaxX(xuanXiang_Check.frame)+8, CGRectGetMinY(xuanXiang_Check.frame), WIDTH-60, Q_CHECK_ICON_WH) Font:Answer_text_font Text:@"无“特殊服务需求”"];
    ...
    //特殊服务需求 1
    UILabel * TeShuFW_Label1=[ZCControl createLabelWithFrame:CGRectMake(15, 30+CGRectGetMaxY(xuanXiang_Label.frame), 44+Title_text_font*5, Title_text_font) Font:Title_text_font Text:@"特殊服务需求1"];
    ...
    if (![PublicFunction isBlankString:_teShuFWModel.need]) {
        my_TeShuFW=_teShuFWModel.need;
        if ([_teShuFWModel.need intValue]==0) {
            
            [xuanXiang_Check setChecked:YES];
            
            TeShuFW_Field1.userInteractionEnabled=NO;
            
            TeShuFW_Field2.userInteractionEnabled=NO;
            
            TeShuFW_Field3.userInteractionEnabled=NO;
            
            TeShuFW_Field4.userInteractionEnabled=NO;
            
            TeShuFW_Field5.userInteractionEnabled=NO;
            
            TeShuFW_Field6.userInteractionEnabled=NO;
        }
    }
...
- (void)didSelectedCheckBox:(QCheckBox *)checkbox checked:(BOOL)checked {
    ...
    if (checkbox.tag==40000) {
        if (checked==1) {
            ...
            TeShuFW_Field1.userInteractionEnabled=YES;
            
            TeShuFW_Field2.userInteractionEnabled=YES;
            
            TeShuFW_Field3.userInteractionEnabled=YES;
            
            TeShuFW_Field4.userInteractionEnabled=YES;
            
            TeShuFW_Field5.userInteractionEnabled=YES;
            
            TeShuFW_Field6.userInteractionEnabled=YES;
...
{
    if ([PublicFunction isBlankString:my_TeShuFW] && [PublicFunction isBlankString:TeShuFW_Field1.text] && [PublicFunction isBlankString:TeShuFW_Field2.text] && [PublicFunction isBlankString:TeShuFW_Field3.text] && [PublicFunction isBlankString:TeShuFW_Field4.text] && [PublicFunction isBlankString:TeShuFW_Field5.text] && [PublicFunction isBlankString:TeShuFW_Field6.text]) {
        UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"请采集“特殊服务需求中的至少一项”，没有，请勾选无" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alertView show];
...
-(void)update_TeShuFW
{
    ...
    [parameter setObject:@"26" forKey:@"tableFlag"];
    
    NSUserDefaults * user = [NSUserDefaults standardUserDefaults];
    [parameter setObject:[user objectForKey:doc_id] forKey:@"doc_id"];
    
    
    if (![PublicFunction isBlankString:my_TeShuFW]) {
        
        [parameter setObject:my_TeShuFW forKey:@"need"];
