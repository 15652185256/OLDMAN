...
#import "NeedYiLiaoWSViewController.h"
...
@interface NeedYiLiaoWSViewController ()<UITextFieldDelegate,UIScrollViewDelegate>
{
...
@implementation NeedYiLiaoWSViewController
...
-(void)loadData
{
    ...
    [_collectViewModel SelectAssessmentResult:self.shenFenZJ tableFlag:@"19"];
...
-(void)createView
{
    ...
    if (![PublicFunction isBlankString:_yiLiaoWSModel.need]) {
        my_YiLiaoWS=_yiLiaoWSModel.need;
        if ([_yiLiaoWSModel.need intValue]==0) {
            [xuanXiang_Check setChecked:YES];
        }
    }
    
    //选项标题
    UILabel * xuanXiang_Label=[ZCControl createLabelWithFrame:CGRectMake(CGRectGetMaxX(xuanXiang_Check.frame)+8, CGRectGetMinY(xuanXiang_Check.frame), WIDTH-60, Q_CHECK_ICON_WH) Font:Answer_text_font Text:@"无“医疗卫生”需求"];
...
#pragma mark - 医疗卫生
    ...
    if ([groupId isEqualToString:@"shengLiZB"]) {
        ...
        shengLiZB=[NSString stringWithFormat:@"%ld",radio.tag-1000];
    }
    ...
    if ([groupId isEqualToString:@"dingQiJX"]) {
        ...
        dingQiJX=[NSString stringWithFormat:@"%ld",radio.tag-2000];
    }
    ...
    if ([groupId isEqualToString:@"dingQiJC"]) {
        ...
        dingQiJC=[NSString stringWithFormat:@"%ld",radio.tag-3000];
    }
    ...
    if ([groupId isEqualToString:@"kangFuDL"]) {
        ...
        kangFuDL=[NSString stringWithFormat:@"%ld",radio.tag-4000];
    }
    ...
    if ([groupId isEqualToString:@"dingQiPT"]) {
        ...
        dingQiPT=[NSString stringWithFormat:@"%ld",radio.tag-5000];
    }
...
{
    if ([PublicFunction isBlankString:my_YiLiaoWS] && [PublicFunction isBlankString:shengLiZB] && [PublicFunction isBlankString:dingQiJX] && [PublicFunction isBlankString:dingQiJC] && [PublicFunction isBlankString:kangFuDL] && [PublicFunction isBlankString:dingQiPT]) {
        UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"请采集“医疗卫生中的至少一项”，没有，请勾选无" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alertView show];
        
    }else if ([shengLiZB intValue]==titleArr.count-1 && [PublicFunction isBlankString:shengLiZB_Field.text]) {
...
-(void)update_YiLiaoWS
{
    ...
    [parameter setObject:@"19" forKey:@"tableFlag"];
    ...
    if (![PublicFunction isBlankString:my_YiLiaoWS]) {
        [parameter setObject:my_YiLiaoWS forKey:@"need"];
    }
