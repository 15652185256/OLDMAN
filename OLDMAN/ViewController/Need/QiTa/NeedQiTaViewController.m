...
#import "NeedQiTaViewController.h"
#import "QiTaModel.h"
...
@interface NeedQiTaViewController ()<UITextFieldDelegate,UIScrollViewDelegate>
{
...
@implementation NeedQiTaViewController
...
-(void)loadData
{
    ...
    [_collectViewModel SelectAssessmentResult:self.shenFenZJ tableFlag:@"25"];
...
-(void)createView
{
    ...
    if (![PublicFunction isBlankString:_qiTaModel.need]) {
        my_QiTa=_qiTaModel.need;
        if ([_qiTaModel.need intValue]==0) {
            [xuanXiang_Check setChecked:YES];
        }
    }
    
    //选项标题
    UILabel * xuanXiang_Label=[ZCControl createLabelWithFrame:CGRectMake(CGRectGetMaxX(xuanXiang_Check.frame)+8, CGRectGetMinY(xuanXiang_Check.frame), WIDTH-60, Q_CHECK_ICON_WH) Font:Answer_text_font Text:@"无“其他”需求"];
...
#pragma mark - 其他
    ...
    if ([groupId isEqualToString:@"ruZhuYL"]) {
        ...
        ruZhuYL=[NSString stringWithFormat:@"%ld",radio.tag-1000];
    }
    ...
    if ([groupId isEqualToString:@"duanQiSQ"]) {
        ...
        duanQiSQ=[NSString stringWithFormat:@"%ld",radio.tag-2000];
    }
    ...
    if ([groupId isEqualToString:@"fuJuPZ"]) {
        ...
        fuJuPZ=[NSString stringWithFormat:@"%ld",radio.tag-3000];
    }
    ...
    if ([groupId isEqualToString:@"gouMaiCQ"]) {
        ...
        gouMaiCQ=[NSString stringWithFormat:@"%ld",radio.tag-4000];
    }
    ...
    if ([groupId isEqualToString:@"gouMaiYW"]) {
        ...
        gouMaiYW=[NSString stringWithFormat:@"%ld",radio.tag-5000];
    }
...
-(void)Save_Button_Click
{
    ...
    if ([PublicFunction isBlankString:my_QiTa] && [PublicFunction isBlankString:ruZhuYL] && [PublicFunction isBlankString:duanQiSQ] && [PublicFunction isBlankString:fuJuPZ] && [PublicFunction isBlankString:gouMaiCQ] && [PublicFunction isBlankString:gouMaiYW]) {
        UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"请采集“其他中的至少一项”，没有，请勾选无" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alertView show];
        
    }else if ([ruZhuYL intValue]==titleArr.count-1 && [PublicFunction isBlankString:ruZhuYL_Field.text]) {
...
-(void)update_QiTa
{
    ...
    [parameter setObject:@"25" forKey:@"tableFlag"];
    ...
    if (![PublicFunction isBlankString:my_QiTa]) {
        [parameter setObject:my_QiTa forKey:@"need"];
    }
