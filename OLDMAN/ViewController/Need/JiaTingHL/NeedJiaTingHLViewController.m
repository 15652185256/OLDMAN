...
#import "NeedJiaTingHLViewController.h"
...
@interface NeedJiaTingHLViewController ()<UITextFieldDelegate,UIScrollViewDelegate>
{
...
@implementation NeedJiaTingHLViewController
...
-(void)loadData
{
    ...
    [_collectViewModel SelectAssessmentResult:self.shenFenZJ tableFlag:@"20"];
...
-(void)createView
{
    ...
    xuanXiang_Check.frame = CGRectMake(15, 20, Q_CHECK_ICON_WH, Q_CHECK_ICON_WH);
    [RootScrollView addSubview:xuanXiang_Check];
    [xuanXiang_Check setChecked:NO];
    
    xuanXiang_Check.tag=40000;
    
    
    if (![PublicFunction isBlankString:_jiaTingHLModel.need]) {
        my_JiaTingHL=_jiaTingHLModel.need;
        if ([_jiaTingHLModel.need intValue]==0) {
            [xuanXiang_Check setChecked:YES];
        }
    }
    
    //选项标题
    UILabel * xuanXiang_Label=[ZCControl createLabelWithFrame:CGRectMake(CGRectGetMaxX(xuanXiang_Check.frame)+8, CGRectGetMinY(xuanXiang_Check.frame), WIDTH-60, Q_CHECK_ICON_WH) Font:Answer_text_font Text:@"无“家庭护理”需求"];
    ...
    //root_View 的初始高度
    float RootScrollView_contentSize=20+CGRectGetMaxY(xuanXiang_Label.frame);
    ...
    for (int j=0; j<titleArray.count; j++) {
        ...
        for (int n=0; n<titleArr.count; n++) {
            ...
            xuanXiang_Button.tag=(j+1)*1000+n;
        ...
        if (checked==1) {
            ...
            for (int j=0; j<titleArray.count; j++) {
                for (int n=0; n<titleArr.count; n++) {
                    ...
                    QRadioButton * xuanXiang_Button=(QRadioButton*)[RootScrollView viewWithTag:(j+1)*1000+n];
...
#pragma mark - 家庭护理
    ...
    if ([groupId isEqualToString:@"qiChuangJQ"]) {
        if (radio.tag-1000==titleArr.count-1) {
            qiChuangJQ_Field.userInteractionEnabled=YES;
        } else {
            qiChuangJQ_Field.userInteractionEnabled=NO;
            qiChuangJQ_Field.text=@"";
        }
        qiChuangJQ=[NSString stringWithFormat:@"%ld",radio.tag-1000];
    }
    
    if ([groupId isEqualToString:@"zhuYu"]) {
        if (radio.tag-2000==titleArr.count-1) {
            zhuYu_Field.userInteractionEnabled=YES;
        } else {
            zhuYu_Field.userInteractionEnabled=NO;
            zhuYu_Field.text=@"";
        }
        zhuYu=[NSString stringWithFormat:@"%ld",radio.tag-2000];
    }
    
    if ([groupId isEqualToString:@"geRenXS"]) {
        if (radio.tag-3000==titleArr.count-1) {
            geRenXS_Field.userInteractionEnabled=YES;
        } else {
            geRenXS_Field.userInteractionEnabled=NO;
            geRenXS_Field.text=@"";
        }
        geRenXS=[NSString stringWithFormat:@"%ld",radio.tag-3000];
    }
    
    if ([groupId isEqualToString:@"ruCeTX"]) {
        if (radio.tag-4000==titleArr.count-1) {
            ruCeTX_Field.userInteractionEnabled=YES;
        } else {
            ruCeTX_Field.userInteractionEnabled=NO;
            ruCeTX_Field.text=@"";
        }
        ruCeTX=[NSString stringWithFormat:@"%ld",radio.tag-4000];
    }
    
    if ([groupId isEqualToString:@"dingQiGH"]) {
        if (radio.tag-5000==titleArr.count-1) {
            dingQiGH_Field.userInteractionEnabled=YES;
        } else {
            dingQiGH_Field.userInteractionEnabled=NO;
            dingQiGH_Field.text=@"";
        }
        dingQiGH=[NSString stringWithFormat:@"%ld",radio.tag-5000];
    }
    
    if ([groupId isEqualToString:@"daXiaoPB"]) {
        if (radio.tag-6000==titleArr.count-1) {
            daXiaoPB_Field.userInteractionEnabled=YES;
        } else {
            daXiaoPB_Field.userInteractionEnabled=NO;
            daXiaoPB_Field.text=@"";
        }
        daXiaoPB=[NSString stringWithFormat:@"%ld",radio.tag-6000];
    }
    
    if ([groupId isEqualToString:@"fuYaoJC"]) {
        if (radio.tag-7000==titleArr.count-1) {
            fuYaoJC_Field.userInteractionEnabled=YES;
        } else {
            fuYaoJC_Field.userInteractionEnabled=NO;
            fuYaoJC_Field.text=@"";
        }
        fuYaoJC=[NSString stringWithFormat:@"%ld",radio.tag-7000];
    }
    
    if ([groupId isEqualToString:@"yaChuangHL"]) {
        if (radio.tag-8000==titleArr.count-1) {
            yaChuangHL_Field.userInteractionEnabled=YES;
        } else {
            yaChuangHL_Field.userInteractionEnabled=NO;
            yaChuangHL_Field.text=@"";
        }
        yaChuangHL=[NSString stringWithFormat:@"%ld",radio.tag-8000];
    }
    
    if ([groupId isEqualToString:@"jiaTingZHZJ"]) {
        if (radio.tag-9000==titleArr.count-1) {
            jiaTingZHZJ_Field.userInteractionEnabled=YES;
        } else {
            jiaTingZHZJ_Field.userInteractionEnabled=NO;
            jiaTingZHZJ_Field.text=@"";
        }
        jiaTingZHZJ=[NSString stringWithFormat:@"%ld",radio.tag-9000];
    }
    
    if ([groupId isEqualToString:@"jiaTingZHZT"]) {
        if (radio.tag-10000==titleArr.count-1) {
            jiaTingZHZT_Field.userInteractionEnabled=YES;
        } else {
            jiaTingZHZT_Field.userInteractionEnabled=NO;
            jiaTingZHZT_Field.text=@"";
        }
        jiaTingZHZT=[NSString stringWithFormat:@"%ld",radio.tag-10000];
    }
    
    if ([groupId isEqualToString:@"jiaTingZHFW"]) {
        if (radio.tag-11000==titleArr.count-1) {
            jiaTingZHFW_Field.userInteractionEnabled=YES;
        } else {
            jiaTingZHFW_Field.userInteractionEnabled=NO;
            jiaTingZHFW_Field.text=@"";
        }
        jiaTingZHFW=[NSString stringWithFormat:@"%ld",radio.tag-11000];
    }
...
{
    if ([PublicFunction isBlankString:my_JiaTingHL] && [PublicFunction isBlankString:qiChuangJQ] && [PublicFunction isBlankString:zhuYu] && [PublicFunction isBlankString:geRenXS] && [PublicFunction isBlankString:ruCeTX] && [PublicFunction isBlankString:dingQiGH] && [PublicFunction isBlankString:daXiaoPB] && [PublicFunction isBlankString:fuYaoJC] && [PublicFunction isBlankString:yaChuangHL] && [PublicFunction isBlankString:jiaTingZHZJ] && [PublicFunction isBlankString:jiaTingZHZT] && [PublicFunction isBlankString:jiaTingZHFW]) {
        UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"请采集“家庭护理中的至少一项”，没有，请勾选无" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alertView show];
        
    }else if ([qiChuangJQ intValue]==titleArr.count-1 && [PublicFunction isBlankString:qiChuangJQ_Field.text]) {
...
-(void)update_JiaTingHL
{
    ...
    [parameter setObject:@"20" forKey:@"tableFlag"];
...
#pragma mark - 家庭护理
    ...
    if (![PublicFunction isBlankString:my_JiaTingHL]) {
        [parameter setObject:my_JiaTingHL forKey:@"need"];
    }
