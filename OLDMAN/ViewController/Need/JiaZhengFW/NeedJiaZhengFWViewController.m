//
//  NeedJiaZhengFWViewController.m
//  OLDMAN
//
...
#import "NeedJiaZhengFWViewController.h"
...
@interface NeedJiaZhengFWViewController ()<UITextFieldDelegate,UIScrollViewDelegate>
{
    ...
    NSString * my_JiaZhengFW;//家政服务 有无
...
@implementation NeedJiaZhengFWViewController
...
-(void)loadData
{
    ...
    [_collectViewModel SelectAssessmentResult:self.shenFenZJ tableFlag:@"23"];
...
-(void)createView
{
    ...
    if (![PublicFunction isBlankString:_jiaZhengFWModel.need]) {
        my_JiaZhengFW=_jiaZhengFWModel.need;
        if ([_jiaZhengFWModel.need intValue]==0) {
            [xuanXiang_Check setChecked:YES];
        }
    }
    
    //选项标题
    UILabel * xuanXiang_Label=[ZCControl createLabelWithFrame:CGRectMake(CGRectGetMaxX(xuanXiang_Check.frame)+8, CGRectGetMinY(xuanXiang_Check.frame), WIDTH-60, Q_CHECK_ICON_WH) Font:Answer_text_font Text:@"无“家政服务”需求"];
...
#pragma mark - 家政服务
    ...
    if ([groupId isEqualToString:@"caiGouRC"]) {
        ...
        caiGouRC=[NSString stringWithFormat:@"%ld",radio.tag-1000];
    }
    ...
    if ([groupId isEqualToString:@"jiaTingQJ"]) {
        ...
        jiaTingQJ=[NSString stringWithFormat:@"%ld",radio.tag-2000];
    }
    ...
    if ([groupId isEqualToString:@"chuangShangYP"]) {
        ...
        chuangShangYP=[NSString stringWithFormat:@"%ld",radio.tag-3000];
    }
    ...
    if ([groupId isEqualToString:@"xiDiZL"]) {
        ...
        xiDiZL=[NSString stringWithFormat:@"%ld",radio.tag-4000];
    }
    ...
    if ([groupId isEqualToString:@"duLiSS"]) {
        ...
        duLiSS=[NSString stringWithFormat:@"%ld",radio.tag-5000];
    }
...
{
    if ([PublicFunction isBlankString:my_JiaZhengFW] && [PublicFunction isBlankString:caiGouRC] && [PublicFunction isBlankString:jiaTingQJ] && [PublicFunction isBlankString:chuangShangYP] && [PublicFunction isBlankString:xiDiZL] && [PublicFunction isBlankString:duLiSS]) {
        UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"请采集“家政服务中的至少一项”，没有，请勾选无" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alertView show];
        
    }else if ([caiGouRC intValue]==titleArr.count-1 && [PublicFunction isBlankString:caiGouRC_Field.text]) {
...
-(void)update_JiaZhengFW
{
    ...
    [parameter setObject:@"23" forKey:@"tableFlag"];
    
    NSUserDefaults * user = [NSUserDefaults standardUserDefaults];
    [parameter setObject:[user objectForKey:doc_id] forKey:@"doc_id"];
    
    
    if (![PublicFunction isBlankString:my_JiaZhengFW]) {
        [parameter setObject:my_JiaZhengFW forKey:@"need"];
    }
