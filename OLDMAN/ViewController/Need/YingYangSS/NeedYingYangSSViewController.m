//
//  NeedYingYangSSViewController.m
//  OLDMAN
//
...
#import "NeedYingYangSSViewController.h"
...
@interface NeedYingYangSSViewController ()<UITextFieldDelegate,UIScrollViewDelegate>
{
    ...
    NSString * my_YingYangSS;//营养膳食 有无
...
@implementation NeedYingYangSSViewController
...
-(void)loadData
{
    ...
    [_collectViewModel SelectAssessmentResult:self.shenFenZJ tableFlag:@"18"];
...
-(void)createNav
{
    ...
    //返回
    UIButton * returnButton=[ZCControl createButtonWithFrame:CGRectMake(0, 0, Return_button_width,Title_text_font) Text:nil ImageName:@"reg_return@2x.png" bgImageName:nil Target:self Method:@selector(returnButtonClick)];
...
-(void)createView
{
    ...
    if (![PublicFunction isBlankString:_yingYangSSModel.need]) {
        my_YingYangSS=_yingYangSSModel.need;
        if ([_yingYangSSModel.need intValue]==0) {
            [xuanXiang_Check setChecked:YES];
        }
    }
    
    //选项标题
    UILabel * xuanXiang_Label=[ZCControl createLabelWithFrame:CGRectMake(CGRectGetMaxX(xuanXiang_Check.frame)+8, CGRectGetMinY(xuanXiang_Check.frame), WIDTH-60, Q_CHECK_ICON_WH) Font:Answer_text_font Text:@"无“营养膳食”需求"];
...
- (void)didSelectedRadioButton:(QRadioButton *)radio groupId:(NSString *)groupId {
    ...
    if ([groupId isEqualToString:@"yingYangSSPG"]) {
        ...
        yingYangSSPG=[NSString stringWithFormat:@"%ld",radio.tag-1000];
    }
    ...
    if ([groupId isEqualToString:@"sanShiZB"]) {
        ...
        sanShiZB=[NSString stringWithFormat:@"%ld",radio.tag-2000];
    }
    ...
    if ([groupId isEqualToString:@"songCan"]) {
        ...
        songCan=[NSString stringWithFormat:@"%ld",radio.tag-3000];
    }
    ...
    if ([groupId isEqualToString:@"jinShiFW"]) {
        ...
        jinShiFW=[NSString stringWithFormat:@"%ld",radio.tag-4000];
    }
...
{
    if ([PublicFunction isBlankString:my_YingYangSS] && [PublicFunction isBlankString:yingYangSSPG] && [PublicFunction isBlankString:sanShiZB] && [PublicFunction isBlankString:songCan] && [PublicFunction isBlankString:jinShiFW]) {
        UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"请采集“营养膳食中的至少一项”，没有，请勾选无" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alertView show];
        
    }else if ([yingYangSSPG intValue]==titleArr.count-1 && [PublicFunction isBlankString:yingYangSSPG_Field.text]) {
...
-(void)update_YingYangSS
{
    ...
    [parameter setObject:@"18" forKey:@"tableFlag"];
...
#pragma mark - 营养膳食
    ...
    if (![PublicFunction isBlankString:my_YingYangSS]) {
        [parameter setObject:my_YingYangSS forKey:@"need"];
    }
