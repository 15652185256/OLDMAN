...
#import "NeedJinJiJYViewController.h"
...
@interface NeedJinJiJYViewController ()<UITextFieldDelegate,UIScrollViewDelegate>
{
...
@implementation NeedJinJiJYViewController
...
-(void)loadData
{
    ...
    [_collectViewModel SelectAssessmentResult:self.shenFenZJ tableFlag:@"21"];
...
-(void)createView
{
    ...
    if (![PublicFunction isBlankString:_jinJiJYModel.need]) {
        my_JinJiJY=_jinJiJYModel.need;
        if ([_jinJiJYModel.need intValue]==0) {
            [xuanXiang_Check setChecked:YES];
        }
    }
    
    //选项标题
    UILabel * xuanXiang_Label=[ZCControl createLabelWithFrame:CGRectMake(CGRectGetMaxX(xuanXiang_Check.frame)+8, CGRectGetMinY(xuanXiang_Check.frame), WIDTH-60, Q_CHECK_ICON_WH) Font:Answer_text_font Text:@"无“紧急救援”需求"];
...
#pragma mark - 紧急救援
    ...
    if ([groupId isEqualToString:@"anZhuangAQ"]) {
        ...
        anZhuangAQ=[NSString stringWithFormat:@"%ld",radio.tag-1000];
    }
    ...
    if ([groupId isEqualToString:@"dingQiJT"]) {
        ...
        dingQiJT=[NSString stringWithFormat:@"%ld",radio.tag-2000];
    }
    ...
    if ([groupId isEqualToString:@"jiaTingKJ"]) {
        ...
        jiaTingKJ=[NSString stringWithFormat:@"%ld",radio.tag-3000];
    }
    ...
    if ([groupId isEqualToString:@"yuanChengAQ"]) {
        ...
        yuanChengAQ=[NSString stringWithFormat:@"%ld",radio.tag-4000];
    }
    ...
    if ([groupId isEqualToString:@"yingJiJY"]) {
        ...
        yingJiJY=[NSString stringWithFormat:@"%ld",radio.tag-5000];
    }
...
{
    if ([PublicFunction isBlankString:my_JinJiJY] && [PublicFunction isBlankString:anZhuangAQ] && [PublicFunction isBlankString:dingQiJT] && [PublicFunction isBlankString:jiaTingKJ] && [PublicFunction isBlankString:yuanChengAQ] && [PublicFunction isBlankString:yingJiJY]) {
        UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"请采集“紧急救援中的至少一项”，没有，请勾选无" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alertView show];
        
    }else if ([anZhuangAQ intValue]==titleArr.count-1 && [PublicFunction isBlankString:anZhuangAQ_Field.text]) {
...
-(void)update_JinJiJY
{
    ...
    [parameter setObject:@"21" forKey:@"tableFlag"];
    
    NSUserDefaults * user = [NSUserDefaults standardUserDefaults];
    [parameter setObject:[user objectForKey:doc_id] forKey:@"doc_id"];
    
    
    if (![PublicFunction isBlankString:my_JinJiJY]) {
        [parameter setObject:my_JinJiJY forKey:@"need"];
    }
