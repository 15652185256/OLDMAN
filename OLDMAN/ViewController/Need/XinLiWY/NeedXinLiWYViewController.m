...
#import "NeedXinLiWYViewController.h"
...
@interface NeedXinLiWYViewController ()<UITextFieldDelegate,UIScrollViewDelegate>
{
    ...
    NSString * my_XinLiWY;//心理及文娱活动 有无
...
@implementation NeedXinLiWYViewController
...
-(void)loadData
{
    ...
    [_collectViewModel SelectAssessmentResult:self.shenFenZJ tableFlag:@"24"];
...
-(void)createView
{
    ...
    if (![PublicFunction isBlankString:_xinLiWYModel.need]) {
        my_XinLiWY=_xinLiWYModel.need;
        if ([_xinLiWYModel.need intValue]==0) {
            [xuanXiang_Check setChecked:YES];
        }
    }
    
    //选项标题
    UILabel * xuanXiang_Label=[ZCControl createLabelWithFrame:CGRectMake(CGRectGetMaxX(xuanXiang_Check.frame)+8, CGRectGetMinY(xuanXiang_Check.frame), WIDTH-60, Q_CHECK_ICON_WH) Font:Answer_text_font Text:@"无“心理及文娱活动”需求"];
...
#pragma mark - 心理及文娱活动
    ...
    if ([groupId isEqualToString:@"shengHuoJN"]) {
        ...
        shengHuoJN=[NSString stringWithFormat:@"%ld",radio.tag-1000];
    }
    ...
    if ([groupId isEqualToString:@"guanHuanFS"]) {
        ...
        guanHuanFS=[NSString stringWithFormat:@"%ld",radio.tag-2000];
    }
    ...
    if ([groupId isEqualToString:@"sheJiaoSS"]) {
        ...
        sheJiaoSS=[NSString stringWithFormat:@"%ld",radio.tag-3000];
    }
    ...
    if ([groupId isEqualToString:@"jiaTingWH"]) {
        ...
        jiaTingWH=[NSString stringWithFormat:@"%ld",radio.tag-4000];
    }
    ...
    if ([groupId isEqualToString:@"xinLiZX"]) {
        ...
        xinLiZX=[NSString stringWithFormat:@"%ld",radio.tag-5000];
    }
    ...
    if ([groupId isEqualToString:@"buLiangQX"]) {
        ...
        buLiangQX=[NSString stringWithFormat:@"%ld",radio.tag-6000];
    }
    ...
    if ([groupId isEqualToString:@"chuXingJH"]) {
        ...
        chuXingJH=[NSString stringWithFormat:@"%ld",radio.tag-7000];
    }
...
{
    if ([PublicFunction isBlankString:my_XinLiWY] && [PublicFunction isBlankString:shengHuoJN] && [PublicFunction isBlankString:guanHuanFS] && [PublicFunction isBlankString:sheJiaoSS] && [PublicFunction isBlankString:jiaTingWH] && [PublicFunction isBlankString:xinLiZX] && [PublicFunction isBlankString:buLiangQX] && [PublicFunction isBlankString:chuXingJH]) {
        UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"请采集“心理及文娱活动中的至少一项”，没有，请勾选无" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alertView show];
        
    }else if ([shengHuoJN intValue]==titleArr.count-1 && [PublicFunction isBlankString:shengHuoJN_Field.text]) {
...
-(void)update_XinLiWY
{
    ...
    [parameter setObject:@"24" forKey:@"tableFlag"];
    
    NSUserDefaults * user = [NSUserDefaults standardUserDefaults];
    [parameter setObject:[user objectForKey:doc_id] forKey:@"doc_id"];
    
    
    if (![PublicFunction isBlankString:my_XinLiWY]) {
        [parameter setObject:my_XinLiWY forKey:@"need"];
    }
