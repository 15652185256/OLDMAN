...
#import "NeedSheQuRJViewController.h"
...
@interface NeedSheQuRJViewController ()<UITextFieldDelegate,UIScrollViewDelegate>
{
...
@implementation NeedSheQuRJViewController
...
-(void)loadData
{
    ...
    [_collectViewModel SelectAssessmentResult:self.shenFenZJ tableFlag:@"22"];
...
-(void)createView
{
    ...
    if (![PublicFunction isBlankString:_sheQuRJModel.need]) {
        my_SheQuRJ=_sheQuRJModel.need;
        if ([_sheQuRJModel.need intValue]==0) {
            [xuanXiang_Check setChecked:YES];
        }
    }
    
    //选项标题
    UILabel * xuanXiang_Label=[ZCControl createLabelWithFrame:CGRectMake(CGRectGetMaxX(xuanXiang_Check.frame)+8, CGRectGetMinY(xuanXiang_Check.frame), WIDTH-60, Q_CHECK_ICON_WH) Font:Answer_text_font Text:@"无“社区日间照料”需求"];
...
#pragma mark - 社区日间照料
    ...
    if ([groupId isEqualToString:@"yingYangSS2"]) {
        ...
        yingYangSS2=[NSString stringWithFormat:@"%ld",radio.tag-1000];
    }
    ...
    if ([groupId isEqualToString:@"geRenSS"]) {
        ...
        geRenSS=[NSString stringWithFormat:@"%ld",radio.tag-2000];
    }
    ...
    if ([groupId isEqualToString:@"fuYaoGL"]) {
        ...
        fuYaoGL=[NSString stringWithFormat:@"%ld",radio.tag-3000];
    }
    ...
    if ([groupId isEqualToString:@"kangFuXL"]) {
        ...
        kangFuXL=[NSString stringWithFormat:@"%ld",radio.tag-4000];
    }
    ...
    if ([groupId isEqualToString:@"yiWuXD"]) {
        ...
        yiWuXD=[NSString stringWithFormat:@"%ld",radio.tag-5000];
    }
    ...
    if ([groupId isEqualToString:@"xinLiSD"]) {
        ...
        xinLiSD=[NSString stringWithFormat:@"%ld",radio.tag-6000];
    }
    ...
    if ([groupId isEqualToString:@"sheJiaoYL"]) {
        ...
        sheJiaoYL=[NSString stringWithFormat:@"%ld",radio.tag-7000];
    }
}
...
{
    if ([PublicFunction isBlankString:my_SheQuRJ] && [PublicFunction isBlankString:yingYangSS2] && [PublicFunction isBlankString:geRenSS] && [PublicFunction isBlankString:fuYaoGL] && [PublicFunction isBlankString:kangFuXL] && [PublicFunction isBlankString:yiWuXD] && [PublicFunction isBlankString:xinLiSD] && [PublicFunction isBlankString:sheJiaoYL]) {
        UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"请采集“社区日间照料中的至少一项”，没有，请勾选无" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alertView show];
        
    }else if ([yingYangSS2 intValue]==titleArr.count-1 && [PublicFunction isBlankString:yingYangSS2_Field.text]) {
...
-(void)update_SheQuRJ
{
    ...
    [parameter setObject:@"22" forKey:@"tableFlag"];
    
    NSUserDefaults * user = [NSUserDefaults standardUserDefaults];
    [parameter setObject:[user objectForKey:doc_id] forKey:@"doc_id"];
    
    
    if (![PublicFunction isBlankString:my_SheQuRJ]) {
        [parameter setObject:my_SheQuRJ forKey:@"need"];
    }
